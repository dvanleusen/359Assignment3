...
Main_Loop:
    ...
    mov   r7, r0                                //moves button input values into r7

    mov   r5, #0                                //clears register r5 as a counter
    mov   r6, #1                                //creates a single bit mask to check value for each button
    ldr   r0, =Response                         //loads response message address
    bl    Print_Message                         //prints response string

    checkLoop:
        tst   r4, r6                            //checks if button is pressed
        beq   update                            //if equal, then that particular button is not pressed

        ldr   r1, =Buttons                      //loads Buttons array address into r1
        ldr   r0, [r1, r5, lsl #2]              //loads corresponding string into r0
        bl    Print_Message                     //prints string

        ldr   r0, =Tab                          //prints a tab
        bl    Print_Message

        cmp   r5, #3                            //checks if it is the 4th button (START) pressed
        ...
        update:
            ...
            cmp   r5, #12                       //we only care about the first 12 bits for now
            blt   checkLoop

        ldr   r0, =NewLine                      //prints new line
        bl    Print_Message

        bl    Main_Loop                         //branches back to main
...
Init_GPIO:
    Loop:
        ...
        subgt r2, #10                           //if greater than 9, sub 10 from r2; r2 is pin# mod 10
        addgt r0, #4                            //if greater than 9, add 4 to r0; r0 is offset
        bgt   Loop                              //if greater than 9, branch back to loop
...
Print_Message:
    push  {lr}
    mov   r1, #0                                //sets loop counter to 0
    ...
    printLoop:
        ...
        bl    WriteStringUART
        pop   {lr}                              //returns to calling code
        bx    lr
...
Read_SNES:
    ...
    pulseLoop:
        ...
        pop {r4, r5, lr}                        //restore register values and return
        bx  lr
...
Wait:
    push {lr}
    ldr  r1, =0x20003004                        //address for low timer counter
